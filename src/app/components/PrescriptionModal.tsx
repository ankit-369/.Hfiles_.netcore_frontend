    ...
    const handleTimingChange = (medIndex: number, timing: string) => {
        setFormData(prev => ({
            ...
            medications: prev.medications.map((med, i) => {
                ...
                return {
                    ...med,
                    timings: [timing] // Only one timing allowed
                };
            })
        }));
    };
                        ...
                        <div key={index} className="space-y-3 sm:space-y-4 border border-gray-200 rounded-lg p-3 sm:p-4">
                            ...
                            <div className="flex flex-col lg:flex-row lg:items-start gap-2 sm:gap-4">
                                ...
                                <div className="flex-1">
                                    <div className="text-xs text-gray-500 mb-2">Select days (choose one or multiple days)</div>
                                    <div className="border border-gray-300 rounded-lg p-2 sm:p-3 max-h-24 sm:max-h-32 overflow-y-auto">
                                        <div className="grid grid-cols-2 sm:grid-cols-1 gap-1">
                                            {dayOptions.map(day => (
                                                <label key={day} className="flex items-center gap-2 py-1 cursor-pointer hover:bg-gray-50 rounded text-xs sm:text-sm">
                                                    <input
                                                        type="checkbox"
                            ...
                            <div className="flex flex-col lg:flex-row lg:items-start gap-2 sm:gap-4">
                                ...
                                <div className="flex-1">
                                    <div className="text-xs text-gray-500 mb-2">Select one timing</div>
                                    <div className="border border-gray-300 rounded-lg p-2 sm:p-3 max-h-24 sm:max-h-32 overflow-y-auto">
                                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-1">
                                            {timingOptions.map(timing => (
                                                <label key={timing} className="flex items-center gap-2 py-1 cursor-pointer hover:bg-gray-50 rounded text-xs sm:text-sm">
                                                    <input
                                                        type="radio"
                                                        name={`timing-${index}`}
                                    ...
                                    {med.timings.length > 0 && (
                                        <div className="mt-2">
                                            <div className="text-xs text-gray-600 mb-1">Selected timing:</div>
                                            <div className="flex flex-wrap gap-1">
